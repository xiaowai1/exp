...
from neuralforecast.losses.pytorch import MAE, DistributionLoss, MSE
...
for target in ['avgcpu', 'avgmem']:
    for file in files:
        ...
        # # 绘制直方图
        # plt.hist(df['resid'], bins=20, color='blue', alpha=0.7)
        # # 添加标题和标签
        ...
        test_seasonal_set = train_seasonal.iloc[train_len:]
        test_trend_set = train_trend.iloc[train_len:]
        test_residual_set = train_residual.iloc[train_len:]

        seasonal_set = train_seasonal.iloc[:train_len]
        trend_set = train_trend.iloc[:train_len]
        residual_set = train_residual.iloc[:train_len]
        ...
        # 将划分好的数据集存储在列表中
        seasonal_datasets.append(seasonal_set)
        trend_datasets.append(trend_set)
        residual_datasets.append(residual_set)
    ...
    # 周期项
    model = iTransformer(h=pred_step,
                         input_size=2,
                         n_series=24,
                         loss=MSE(),
                         val_check_steps=1,
    ...
    # 残差项
    model2 = iTransformer(h=pred_step,
                          input_size=2,
                          n_series=24,
                          loss=MSE(),
                          val_check_steps=1,
    ...
    #         save_dataset=True)
    # 趋势项
    model3 = LSTM(h=pred_step, input_size=96,
                  loss=DistributionLoss(distribution='Normal', level=[80, 90]), max_steps=100)
    ...
    for key in test_seasonal_dictionary:
        ...
        for i in range(0, len(test_seasonal_df), pred_step):
            test_seasonal_chunk = test_seasonal_df.iloc[i:i + pred_step]
            test_trend_chunk = test_trend_df.iloc[i:i + pred_step]
            test_residual_chunk = test_residual_df.iloc[i:i + pred_step]
            if len(test_seasonal_chunk) < pred_step:
        ...
        df.to_csv(f'./experiment1/results/{target}/seasonal/{key}-{target}-Forecast.csv', index=False)
        plt.figure()
        ...
        plt.ylabel(f'{target}')
        plt.legend()
        plt.savefig(f"./experiment1/images/seasonal/{key}-{target}.png")
        # 趋势项
        df = trend_plot_df[['y', 'LSTM']].rename(columns={'y': f'{target}-true', 'LSTM': 'forecast'})
        df.to_csv(f'./experiment1/results/{target}/trend/{key}-{target}-Forecast.csv', index=False)
        plt.figure()
        plt.style.use('ggplot')
        ...
        plt.ylabel(f'{target}')
        plt.legend()
        plt.savefig(f"./experiment1/images/trend/{key}-{target}.png")
        # 残差项
        df = residual_plot_df[['y', 'iTransformer']].rename(columns={'y': f'{target}-true', 'iTransformer': 'forecast'})
        df.to_csv(f'./experiment1/results/{target}/residual/{key}-{target}-Forecast.csv', index=False)
        plt.figure()
        plt.style.use('ggplot')
        ...
        plt.ylabel(f'{target}')
        plt.legend()
        plt.savefig(f"./experiment1/images/residual/{key}-{target}.png")
