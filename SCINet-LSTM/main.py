...
class SCINetinitialization():
    ...
    def build_model(self, args):
        # models = SCINet.Model(self.args).float()
                ...
                with torch.no_grad():
                    for _, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(pred_loader):
                        ...
                        for i in range(args.pred_len):
                            ...
                            preds.append(outputs[0][i][outputs.shape[2] - index])  # 取最后一个预测值即对应target列
                        print(outputs)

            # 保存结果
            if args.rollingforecast:
                df = pd.DataFrame({f'{target.capitalize()}real': pre_data['{}'.format(target)][:len(preds)], 'forecast': preds})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--models', type=str, default='SCINet', help='Model name')
